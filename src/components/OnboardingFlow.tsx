      ...
      <DialogContent className="sm:max-w-md">
        ...
        <DialogHeader>
          <div className="flex items-center justify-between">
            ...
            <span className="text-sm text-muted-foreground">Schritt {getStepNumber()}/3</span>
          </div>
        </DialogHeader>
        ...
        {currentStep === "lecture" && (
          <div className="space-y-4">
            <p className="text-sm text-muted-foreground">
              Lass uns deine erste Vorlesung anlegen. Das dauert nur 10 Sekunden!
            </p>
            ...
            <div className="flex gap-2">
              <Button onClick={handleLectureSubmit} className="flex-1" disabled={isSubmitting}>
                {isSubmitting ? "Wird erstellt..." : "Weiter"}
              </Button>
              <Button onClick={handleSkip} variant="ghost" disabled={isSubmitting}>
                Überspringen
              </Button>
            </div>
          </div>
        ...
        {currentStep === "exam" && (
          <div className="space-y-4">
            ...
            <div className="flex gap-2">
              <Button onClick={handleExamSubmit} className="flex-1" disabled={isSubmitting}>
                {isSubmitting ? "Wird erstellt..." : "Weiter"}
              </Button>
              <Button onClick={handleSkip} variant="ghost" disabled={isSubmitting}>
                Überspringen
            ...
            <div className="rounded-lg border bg-muted/50 p-4 space-y-3">
              <div className="flex items-start gap-3">
                ...
                <div className="flex-1">
                  ...
                  <p className="text-xs text-muted-foreground mt-1">
                    Hier siehst du alle wichtigen Termine und Aufgaben für heute auf einen Blick.
                  </p>
                </div>
              </div>
              ...
              <div className="flex items-start gap-3">
                ...
                <div className="flex-1">
                  ...
                  <p className="text-xs text-muted-foreground mt-1">
                    Mit dem blauen Plus-Button unten rechts kannst du jederzeit schnell etwas anlegen.
                  </p>
                </div>
              </div>
            </div>
        ...
        <div className="flex gap-1 justify-center">
          <div className={`h-1.5 w-8 rounded-full transition-colors ${currentStep === "lecture" ? "bg-blue-600" : "bg-muted"}`} />
          <div className={`h-1.5 w-8 rounded-full transition-colors ${currentStep === "exam" ? "bg-blue-600" : "bg-muted"}`} />
          <div className={`h-1.5 w-8 rounded-full transition-colors ${currentStep === "today-view" ? "bg-blue-600" : "bg-muted"}`} />
        </div>
      </DialogContent>
    </Dialog>
  );
